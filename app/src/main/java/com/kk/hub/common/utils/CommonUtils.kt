...
import com.kk.hub.common.image.BlurTransformation
import com.kk.hub.common.image.GSYImageLoaderManager
import com.kk.hub.common.image.GSYLoadOption
...
object CommonUtils {
    ...
    fun getNewsTimeStr(date: Date?): String {
        ...
        return when {
            ...
            subTime < SECONDS_LIMIT -> Math.round(subTime / MILLIS_LIMIT).toString() + " " + App.instance.getString(
                R.string.secondAgo
            )
            subTime < MINUTES_LIMIT -> Math.round(subTime / SECONDS_LIMIT).toString() + " " + App.instance.getString(
                R.string.minutesAgo
            )
            subTime < HOURS_LIMIT -> Math.round(subTime / MINUTES_LIMIT).toString() + " " + App.instance.getString(
                R.string.hoursAgo
            )
            subTime < DAYS_LIMIT -> Math.round(subTime / HOURS_LIMIT).toString() + " " + App.instance.getString(
                R.string.daysAgo
            )
